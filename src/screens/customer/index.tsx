...
function CustomerScreen() {
  ...
  const [filterListCustomer, setFilterListCustomer] = useState<TCustomer[]>([]);
  ...
  useEffect(() => {
    if (debounce) {
      ...
      const filter = customers.filter((value: TCustomer) => {
      ...
      setFilterListCustomer(filter)
    } else {
      setFilterListCustomer([])
    }
  }, [debounce])
  ...
  return (
    <Screen
      ...
      onIconRightPress={() => navigation.navigate('CreateEditCustomer')}
    >
      <View style={[styles.container]}>
        <FlatList
          initialNumToRender={5}
          maxToRenderPerBatch={5}
          ListHeaderComponent={customers.length > 0 ? <SearchBox setSearch={setSearch} /> : <></>}
          ListEmptyComponent={<EmptyData color={Colors.mainColor} label={debounce ? 'Không tìm thấy khách hàng này' : 'Chưa có dữ liệu!'} />}
          ListFooterComponent={<View style={{ height: 30 }}></View>}
          style={[styles.list]}
          data={debounce ? filterListCustomer : customers}
